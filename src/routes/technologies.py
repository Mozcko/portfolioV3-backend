...
@router.post("/", response_model=technology_schema.Technology, dependencies=[Depends(get_current_admin_user)])
...
@router.put("/{technology_id}", response_model=technology_schema.Technology, dependencies=[Depends(get_current_admin_user)])
def update_technology(
    technology_id: int,
    db: Session = Depends(get_db),
    name: Optional[str] = Form(None),
    color: Optional[str] = Form(None),
    icon_file: Optional[UploadFile] = File(None, alias="icon") # <-- El archivo es opcional
):
    update_data = {}
    if name is not None:
...
@router.delete("/{technology_id}", response_model=technology_schema.Technology, dependencies=[Depends(get_current_admin_user)])
def delete_technology(technology_id: int, db: Session = Depends(get_db)):
    db_technology = technologies_service.delete_technology(db=db, technology_id=technology_id)
    if db_technology is None:
        raise HTTPException(status_code=404, detail="Technology not found")
    return db_technology